...
const ImageForm: FC<ImageFormProps> = ({ incrementEntries, user }) => {
	...
	const calculateFaceBox = () => {
		...
		const faces = state.faceData.outputs[0].data.regions.map(
			region => region.region_info.bounding_box
		);

		if (!faces || !imgRef.current) return;

		const imageWidth = Number(imgRef.current.width);
		const imageHeight = Number(imgRef.current.height);
				...
				{boxes.map((face, index) => (
					<div
						...
						style={{
							...
							width: `${imageWidth - (face.rightCol + face.leftCol)}px`,
							height: `${imageHeight - (face.bottomRow + face.topRow)}px`
						}}
					/>
				))}
			</>
	...
	const handleSubmit = async () => {
		...
		const res = await fetch(baseUrl + '/fetchimage', {
			method: 'POST',
			headers: {
		...
		if (res.status === 200) {
			const faceData = (await res.json()) as FaceData;
			...
			const res2 = await fetch(baseUrl + '/image', {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
					'Access-Control-Allow-Origin': '*'
			...
			if (res2.status === 200) incrementEntries();

			return;
		}

		if (res.status === 500) toggleState(SET_IS_IMAGE);
	};
		...
		<>
			<div className='image-form-body'>
				...
				<div className='image-form-image-container'>
					<img
						id='image-form-imageBody'
						ref={imgRef}
						src={state.input}
						alt=''
					/>
					{state.faceData ? calculateFaceBox() : ''}
				</div>
				{state.activeBoolean ? (
					<MessageBar
						type='error'
